...
```{r PTM_raw_table}
phosphosites_raw <- read_tsv(file="/Users/jesusav/Documents/Cats/Article/Data/2025/ToxoDB_29072025_Phosphosites_raw.txt")
colnames(phosphosites_raw) <- c("Gene_ID","source_id", "Modified_Residues","Total_Modified_Residues", "Total_Modifications_By_Type")
phosphosites_raw <- phosphosites_raw %>% select(-Total_Modifications_By_Type)
phosphosites_raw <- phosphosites_raw %>% mutate(strain=gsub("_.*?$", "", Gene_ID)) %>% filter(strain!="TGGT1") %>% select(-strain)

max(phosphosites_raw$Total_Modified_Residues)
phosphosites <- phosphosites_raw %>% select(-Total_Modified_Residues) %>% separate_rows(Modified_Residues, sep = ",")
phosphosites <- phosphosites %>% separate(Modified_Residues, c("aa_res", "aa_pos"), sep=":")
#write_tsv(phosphosites, file="/Users/jesusav/Documents/Cats/Article/Data/2025/ToxoDB_29072025_Phosphosites.tsv")

```


```{r phosphosites_survey}
phosphosites <- read_tsv(file="/Users/jesusav/Documents/Cats/Article/Data/2025/ToxoDB_29072025_Phosphosites.tsv")

matches_phosphosite_survey <- raw_matches %>% filter(ToxoDB_ID %in% phosphosites$Gene_ID) %>% 
  select(ToxoDB_ID, Motif_Name, Match_N, Motif_Instance, Motif_sSite) %>%
  mutate(Motif_eSite=Motif_sSite+nchar(Motif_Instance)-1)
matches_phosphosite_protein <- matches_phosphosite_survey %>% select(ToxoDB_ID) %>% unique()

matches_phosphosite <- matches_phosphosite_survey[0,]
matches_phosphosite$Modification_n <- ""
...
matches_phosphosite <- matches_phosphosite %>% filter(Modification_n>0)
...
uniprot_domains_raw <- read_tsv(file="/Users/jesusav/Documents/Cats/Article/Data/2025/uniprotkb_taxonomy_id_508771_2025_07_29.tsv")
...
#write_tsv(uniprot_domains %>% select(-Protein_names), file="/Users/jesusav/Documents/Cats/Article/Data/2025/UniProt_29072025_domains.tsv")
```
...
#write_tsv(matches_domains, file="/Users/jesusav/Desktop/July2025_MotifMatches_domains.tsv")


```

```{r raw_pLDDT}
uniprot_ids <- read_tsv(file="/Users/jesusav/Documents/Cats/Article/Data/2025/Genes_UniProt.txt") 
...
pLDDT_values <- read_table(file="/Users/jesusav/Documents/Cats/Article/Data/2025/all_models_pLDDT_values.txt", col_names = c("File", "AA", "Chain", "residue_n","plDDT")) 
...
#write_tsv(matches_pLDDT_all, file="/Users/jesusav/Desktop/ToxoME49_69.ELM2025_MotifMatches_pLDDT.tsv")
#write_tsv(matches_pLDDT_all, file="/Users/jesusav/Desktop/July2025_MotifMatches_pLDDT.tsv")
matches_pLDDT_all <- read_tsv(file="/Users/jesusav/Documents/Cats/Article/Data/2025/ToxoME49_69.ELM2025_MotifMatches_pLDDT.tsv")
...
dssp_values <- read_table(file="/Users/jesusav/Documents/Cats/Article/Data/2025/all_model_dssp_values.txt", col_names = c("File", "residue_n", "Chain", "AA","total_accessibility")) 
...
#write_tsv(matches_acc_all, file="/Users/jesusav/Desktop/July2025_MotifMatches_RSA.tsv")
matches_acc_all <- read_tsv(file="/Users/jesusav/Documents/Cats/Article/Data/2025/July2025_MotifMatches_RSA.tsv")
